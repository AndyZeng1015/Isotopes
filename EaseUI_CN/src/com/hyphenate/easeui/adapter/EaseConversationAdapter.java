...
public class EaseConversationAdapter extends ArrayAdapter<EMConversation> {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (conversation.getType() == EMConversationType.GroupChat) {
        	...
        	if(conversation.getExtField() != null && !conversation.getExtField().equals("")){
        		Glide.with(getContext()).load(conversation.getExtField().split("\\|")[1]).diskCacheStrategy(DiskCacheStrategy.ALL).placeholder(R.drawable.ease_default_avatar).into(holder.avatar);
        		holder.name.setText(conversation.getExtField().split("\\|")[0]);
        	}
            holder.motioned.setVisibility(View.GONE);
        }
